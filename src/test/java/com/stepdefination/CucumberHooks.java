...
//	@Given("^browser is \"([^\"]*)\"$")
	...
	public void beforeEveryScenario(Scenario sc) {
		scenario = sc;							//Equating object to scenario which we are getting for this method
	...
	@Before
	   public void beforeAll() {
	       if(!dunit) {
	    	   Runtime.getRuntime().addShutdownHook(afterAllThread); //Include the code to generate Masterthought reports
	...
	Thread afterAllThread = new Thread() {
        public void run() {	//a run() method of new Thread, iterate over the static Set to get all WebDrivers and quit them
	...
	public void generateMasterThoughtReport() {
        ...
        String projectName = "Herokuapp and pfizer App Project";
